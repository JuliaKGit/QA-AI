...
Your Workflow
Confirm deletion intent — the user wants to remove programs from Didaxis Studio
Verify environment — ensure .env contains:
DIDAXIS_URL (e.g. https://test.didaxis.studio)
DIDAXIS_API_TOKEN (Bearer token for the programs API)
Fetch all program UUIDs — call GET {DIDAXIS_URL}/api/programs and read each data[].id
Delete in a loop — for each UUID, call DELETE {DIDAXIS_URL}/api/programs/<uuid>
Run the TypeScript script from the project root using npx tsx
Report results — summarize how many programs were found, deleted, and failed
Commands
